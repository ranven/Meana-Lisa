#!/usr/bin/env python3
"""
Script to train a logistic regression model for predicting centuries from vectorized palettes.

This script:
1. Connects to MongoDB and extracts vectorizedPalette and objectCentury data
2. Prepares feature matrix X (n_samples × VECTORIZED_PALETTE_LENGTH) and labels y (centuries)
...
Usage: python trainModelCentury.py
"""
...
VECTORIZED_PALETTE_LENGTH = 15  # Expected length of vectorizedPalette arrays
...
def setup_logging():
    ...
    log_filename = LOGS_DIR / f"century_model_training_{timestamp}.log"
...
def extract_training_data() -> Tuple[np.ndarray, np.ndarray]:
    """
    Extract vectorizedPalette and objectCentury data from MongoDB.
    Filters out centuries with fewer than 2 samples to avoid stratified split errors.
    
    Returns:
        Tuple of (X, y) where:
        - X: Feature matrix (n_samples × VECTORIZED_PALETTE_LENGTH) of vectorized palettes
        - y: Labels array (n_samples) of centuries
    """
    ...
    try:
        ...
        # Query for documents that have both vectorizedPalette and objectCentury
        query = {
            "vectorizedPalette": {"$exists": True},
            "objectCentury": {"$exists": True, "$ne": None}
        }
        
        # Get total count for progress tracking
        total_docs = collection.count_documents(query)
        logger.info(f"Found {total_docs} documents with both vectorizedPalette and objectCentury")
        ...
        for doc in collection.find(query):
            try:
                ...
                object_century = doc.get("objectCentury")
                ...
                if not isinstance(object_century, int):
                    logger.warning(f"Document {doc['_id']} has invalid objectCentury format, skipping")
                    skipped += 1
                    continue
                
                # Convert to numpy arrays and add to training data
                X.append(vectorized_palette)
                y.append(object_century)
        ...
        # Analyze century distribution and filter out rare centuries
        unique_centuries, counts = np.unique(y, return_counts=True)
        logger.info(f"Century distribution:")
        for century, count in zip(unique_centuries, counts):
            logger.info(f"  {century}: {count} samples")
        
        # Filter out centuries with fewer than 2 samples
        min_samples_per_class = 2
        valid_centuries = unique_centuries[counts >= min_samples_per_class]
        filtered_centuries = unique_centuries[counts < min_samples_per_class]
        
        if len(filtered_centuries) > 0:
            logger.warning(f"Filtering out centuries with < {min_samples_per_class} samples: {filtered_centuries.tolist()}")
            logger.warning(f"Total samples being filtered out: {sum(counts[counts < min_samples_per_class])}")
        
        # Create mask for valid samples
        valid_mask = np.isin(y, valid_centuries)
        ...
        logger.info(f"  Valid centuries: {sorted(valid_centuries.tolist())}")
        logger.info(f"  Number of valid centuries: {len(valid_centuries)}")
        
        if len(valid_centuries) < 2:
            logger.error(f"Not enough valid centuries ({len(valid_centuries)}) for classification. Need at least 2.")
            return np.array([]), np.array([])
...
def train_logistic_regression_model(X: np.ndarray, y: np.ndarray) -> Tuple[LogisticRegression, StandardScaler, LabelEncoder]:
    """
    Train logistic regression model for century prediction.
    ...
    try:
        ...
        # Encode centuries (convert century values to integer labels)
...
def save_model_and_preprocessors(model: LogisticRegression, scaler: StandardScaler, encoder: LabelEncoder):
    ...
    try:
        ...
        # Save model
        model_path = models_dir / f"century_predictor_model_{timestamp}.pkl"
        with open(model_path, 'wb') as f:
            pickle.dump(model, f)
        logger.info(f"Model saved to: {model_path}")
        
        # Save scaler
        scaler_path = models_dir / f"century_predictor_scaler_{timestamp}.pkl"
        with open(scaler_path, 'wb') as f:
            pickle.dump(scaler, f)
        logger.info(f"Scaler saved to: {scaler_path}")
        
        # Save encoder
        encoder_path = models_dir / f"century_predictor_encoder_{timestamp}.pkl"
        ...
        metadata_path = models_dir / f"century_predictor_metadata_{timestamp}.pkl"
...
def predict_century_from_raw_palette(palette: List[List], model: LogisticRegression, 
    ...
    Returns:
        Tuple of (predicted_century, confidence_score)
    """
    logger = logging.getLogger(__name__)
    ...
    try:
        ...
        return predict_century_from_palette(vectorized_palette, model, scaler, encoder)
        
    except Exception as e:
        logger.error(f"Error predicting century from raw palette: {str(e)}")
        raise

def predict_century_from_palette(vectorized_palette: List[float], model: LogisticRegression, 
                               scaler: StandardScaler, encoder: LabelEncoder) -> Tuple[int, float]:
    """
    Predict century from a vectorized palette.
    ...
    try:
        ...
        logger.error(f"Error predicting century: {str(e)}")
        raise
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Starting century prediction model training")
    ...
    try:
        ...
        predicted_century, confidence = predict_century_from_palette(sample_palette, model, scaler, encoder)
        actual_century = y[0]
        
        logger.info(f"Sample prediction:")
        logger.info(f"  Actual century: {actual_century}")
